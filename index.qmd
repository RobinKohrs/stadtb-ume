...
<div class='highlight'>  
Insgesamt sind im Baumkataster der Stadt wien <strong>224.025</strong> Bäume eingetragen.
</div>
...
geo_bk %>% 
  ...
  slice_head(n=10) %>% 
  mutate(
    GATTUNG_AR = fct_reorder(GATTUNG_AR, n, .desc = T)
  ) %>% 
  ggplot(aes(GATTUNG_AR, n)) +
  geom_col() +
  labs(x=NULL, y=NULL) +
...
```{r}
d_trees_per_person %>% 
  ggplot(aes(pop, trees_per_person)) +
  geom_point() +
  scale_y_log10() +
  theme_light() -> p
...
<div class='highlight'>  
  Anscheinend wurde der derzeit häufigst existierende Baum in Wien, der Spitzahorn,
</div>
...
data_with_decade %>% 
  ...
  labs(y = "Bäume pro Dekade", x="Dekade", caption = "Ausgewählt wurden die Top-7 Baumarten in der Dekade 2020") +
  scale_x_continuous(labels=function(x)paste0(x, "'s"), breaks=seq(1850,2020, by=20)) +
  geom_line(show.legend = T, linewidth=3) +
  geom_point(size=4) +
...
data_with_decade %>% 
  group_by(GATTUNG_AR, decade) %>% 
  summarise(
    n_per_decade = n()
  ) %>% 
  ungroup() %>% 
  group_by(GATTUNG_AR) %>% 
  mutate(index = if_else(decade == min(decade),
                         1,
                         n_per_decade / n_per_decade[decade == min(decade)])) %>% 
  mutate(
    is_spitzahorn = if_else(GATTUNG_AR == "Acer platanoides (Spitzahorn)", T, F)
  ) -> data_spitzahorn

# data_spitzahorn %>% filter(is_spitzahorn) %>% View

data_spitzahorn %>% 
  ggplot(aes(decade, index, color=is_spitzahorn, group=GATTUNG_AR, alpha=is_spitzahorn)) +
  geom_line(linewidth=3) +
  labs(title="Der Spitzhorn wurde besonders in den 90ern gepflanzt", color=NULL, alpha=NULL) +
  guides(alpha="none") +
  theme_light(base_size = 30) +
  scale_color_manual(values=c("FALSE" = "grey", "TRUE" = "darkred"), labels=c("TRUE" = "Spitzahorn", "FALSE" = "anderen")) + theme(legend.position = "bottom")
